...
class xfilesharing(cloudservice.cloudservice):
    ...
    def getVideosList(self, folderID=0, cacheType=0):
        ...
        if folderID == 0:
            ...
            url = url+'?op=my_files&fld_id='+folderID+'&per_page=100000'
        # ici pour modifier le nombre de ligne par page pour UpToBox
        ...
        if True:
            ...
            for r in re.finditer('<a id="([^\"]+)" href="([^\"]+)">([^\<]+)</a>' ,
                ...
                # streaming
                videos[fileName] = {'url': 'plugin://plugin.video.uptoboxstreaming?mode=streamURL&instance='+self.instanceName+'&url=' + url, 'mediaType' : self.MEDIA_TYPE_VIDEO}
            ...
            for r in re.finditer('<a href=".*?fld_id=([^\"]+)" [^\s]+>([^\<]+)</a></td>' ,
                    ...
                    if int(folderID) != 0 and folderName != '&nbsp;. .&nbsp;':
                        videos[folderName] = {'url': 'plugin://plugin.video.uptoboxstreaming?mode=folder&instance='+self.instanceName+'&folderID=' + folderID, 'mediaType' : self.MEDIA_TYPE_FOLDER}
